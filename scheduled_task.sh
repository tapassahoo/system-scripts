...
python temp_execution_moribs_driver_linear_molecule_n${var_molecule}_beta${var_parameter}_nskip${var_preskip}.py > ${HOME}/final-pigs-outputs-for-plotting/moribs_${var_job_type}_n${var_molecule}_beta${var_parameter}_nskip${var_preskip}.log &
date 
echo -e "\n"
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
